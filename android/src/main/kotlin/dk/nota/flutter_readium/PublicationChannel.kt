...
internal const val publicationChannelName = "dk.nota.flutter_readium/Publication"
...
private suspend fun openPublication(
  ...
  try {
    ...
    Log.d(TAG, "Created publication = $pub")
    publication = pub
    ...
    CoroutineScope(Dispatchers.Main).launch {
      ...
      when (call.method) {
        "fromPath" -> {
          ...
          val mediaType = parseMediaType(args[1])
          val url = AbsoluteUrl("file://$path") ?: run {
            Log.e(TAG, "fromPath: Invalid URL")
            return@launch
          }
          // TODO: provide mediaType to assetRetriever
          val asset: Asset? = readium!!.assetRetriever.retrieve(url)
